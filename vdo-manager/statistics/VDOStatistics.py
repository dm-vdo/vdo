...
class VDOStatistics(StatStruct):
	def __init__(self, name="VDOStatistics", **kwargs):
		super(VDOStatistics, self).__init__(name, [
			...
			Uint64Field("oneKBlocksUsed", available = "((not $inRecoveryMode) and ($mode != \"read-only\"))", label = "1K-blocks used", derived = "($dataBlocksUsed + $overheadBlocksUsed) * $blockSize // 1024"),
			Uint64Field("oneKBlocksAvailable", derived = "($physicalBlocks - $dataBlocksUsed - $overheadBlocksUsed) * $blockSize // 1024", label = "1K-blocks available", available = "((not $inRecoveryMode) and ($mode != \"read-only\"))"),
