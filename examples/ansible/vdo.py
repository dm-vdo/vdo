...
options:
    ...
    device:
        ...
        required: true if 'state' == 'present'
    logicalsize:
        description:
    ...
    physicalthreads:
        ...
        required: false
notes:
  - VDO volumes with the names 'Enabled' or 'Server' are discouraged.
'''
...
def add_vdooptions(params):
  ...
  if (('blockmapcachesize' in params)
     and (params['blockmapcachesize'] is not None)):
  ...
  if ('physicalthreads' in params) and (params['physicalthreads'] is not None):
...
def run_module():
  ...
  if (desiredvdo in vdolist) and (state == 'present'):
    ...
    for statfield in statusparamkeys:
      ...
      vdomodparamtrans[statfield] = vdokeytrans[statfield]
    ...
    for paramkey in currentvdoparams.keys():
      currentparams[vdomodparamtrans[paramkey]] = currentvdoparams[paramkey]
    ...
    if diffparams:
      ...
      rc, _, err = module.run_command("%s modify --name=%s %s"
                                      % (vdo_cmd, desiredvdo, vdocmdoptions))
      if rc == 0:
        result['changed'] = True
      else:
      ...
      if 'deduplication' in diffparams.keys():
        ...
        if dedupemod == 'enabled':
          result['dedupeopt'] = dedupemod
          rc, _, err = module.run_command("%s enableDeduplication --name=%s"
                                          % (vdo_cmd, desiredvdo))
      ...
      if 'compression' in diffparams.keys():
        ...
        if compressmod == 'enabled':
          result['compressopt'] = compressmod
          rc, _, err = module.run_command("%s enableCompression --name=%s"
                                          % (vdo_cmd, desiredvdo))
    ...
    for statfield in sizeparamkeys:
      ...
      vdosizeparamtrans[statfield] = vdokeytrans[statfield]

    sizeparams = {}
    for paramkey in currentsizeparams.keys():
      sizeparams[vdosizeparamtrans[paramkey]] = currentsizeparams[paramkey]
    ...
    # call.
    if (((vdoactivatestatus == 'enabled') 
         or (module.params['activated'] == 'yes'))
