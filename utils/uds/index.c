...
static int load_index(struct index *index, bool allow_replay)
{
	...
	uds_log_info("loaded index from chapter %" PRIu64 " through chapter %" PRIu64,
		     index->oldest_virtual_chapter,
		     last_checkpoint_chapter);
...
int save_index(struct index *index)
{
	...
	if (result != UDS_SUCCESS) {
		...
		uds_log_info("finished save (vcn %" PRIu64 ")",
			     index->last_checkpoint);
	}
	return result;
}
		...
		uint64_t open_chapter_number)
{
	...
	uds_log_info("beginning %s (vcn %" PRIu64 ")",
...
int replay_volume(struct index *index, uint64_t from_vcn)
{
	...
	uds_log_info("Replaying volume from chapter %" PRIu64 " through chapter %" PRIu64,
		     from_vcn,
		     upto_vcn);
	...
	for (vcn = from_vcn; vcn < upto_vcn; ++vcn) {
		...
		if (check_for_suspend(index)) {
			uds_log_info("Replay interrupted by index shutdown at chapter %" PRIu64,
				     vcn);
	...
	if (new_ipm_update != old_ipm_update) {
		uds_log_info("replay changed index page map update from %" PRIu64 " to %" PRIu64,
			     old_ipm_update,
			     new_ipm_update);
	}
