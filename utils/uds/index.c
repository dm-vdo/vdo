...
static int load_index(struct index *index, bool allow_replay)
{
	...
	log_info("loaded index from chapter %" PRIu64 " through chapter %" PRIu64,
		 index->oldest_virtual_chapter,
...
int save_index(struct index *index)
{
	...
	if (result != UDS_SUCCESS) {
		...
		log_info("finished save (vcn %" PRIu64 ")",
			 index->last_checkpoint);
	}
	return result;
}
		...
		uint64_t open_chapter_number)
{
	...
	log_info("beginning %s (vcn %" PRIu64 ")", what,
		 index->last_checkpoint);
}
...
int replay_volume(struct index *index, uint64_t from_vcn)
{
	...
	log_info("Replaying volume from chapter %" PRIu64 " through chapter %" PRIu64,
		 from_vcn,
		 upto_vcn);
	...
	for (vcn = from_vcn; vcn < upto_vcn; ++vcn) {
		if (check_for_suspend(index)) {
			log_info("Replay interrupted by index shutdown at chapter %" PRIu64,
				 vcn);
	...
	if (new_ipm_update != old_ipm_update) {
		log_info("replay changed index page map update from %" PRIu64 " to %" PRIu64,
			 old_ipm_update,
			 new_ipm_update);
	}
