 ...
 * #udsLoadLocalIndex, #udsRebuildLocalIndex, or #udsAttachGridIndex.
 *
 * Each context is associated with a namespace. Entries indexed in one
 * namespace will not deduplicate with entries indexed in another namespace.
 * Use a namespace parameter of <code>NULL</code> if you do not wish to use a
 * namespace, which allows backward-compatibility with older,
 * non-namespace-enabled APIs.
 *
 ...
 * will return #UDS_DISABLED. In that case, close the block context and then
 * close the index session. If the index is local then reload or rebuild it.
 * If it is a grid index, reattach the index session to it. Then call
 * #udsOpenBlockContext to open a fresh block context.
 *
 * @param [in] session       The index session
 * @param [in] nspace        A namespace specifier, or <code>NULL</code> for
 *                           no namespace
