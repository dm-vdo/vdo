 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/uds-releases/krusty/src/uds/indexZone.c#25 $
 */
	...
	int result =
		ASSERT(((zone->newest_virtual_chapter -
			 zone->oldest_virtual_chapter) <= chapters_per_volume),
		       "newest (%" PRIu64 ") and oldest (%" PRIu64 ") virtual chapters less than or equal to chapters per volume (%u)",
		       zone->newest_virtual_chapter,
		       zone->oldest_virtual_chapter,
		       chapters_per_volume);
	if (result != UDS_SUCCESS) {
...
int open_next_chapter(struct index_zone *zone, Request *request)
{
	...
	uds_log_debug("closing chapter %" PRIu64 " of zone %d after %u entries (%u short)",
		      zone->newest_virtual_chapter,
		      zone->id,
