				   ...
				   struct delta_index_page *chapter_index_page)
{
	...
	log_warning("Index page map updated to %" PRIu64,
		    get_last_update(volume->index_page_map));
	log_warning("Page map expects that chapter %u page %u has range %u to %u, but chapter index page has chapter %" PRIu64 " with range %u to %u",
...
{
	log_debug("forgetting chapter %" PRIu64, virtual_chapter);
	unsigned int physical_chapter =
		map_to_physical_chapter(volume->geometry, virtual_chapter);
	lock_mutex(&volume->read_threads_mutex);
			 ...
			 uint64_t *virtual_chapter_number)
{
	...
	for (i = 0; i < geometry->index_pages_per_chapter; ++i) {
		...
		if (last_vcn == UINT64_MAX) {
			...
			uds_log_error("inconsistent chapter %u index page %u: expected vcn %" PRIu64 ", got vcn %" PRIu64,
	...
	if (chapter_number != map_to_physical_chapter(geometry, last_vcn)) {
		uds_log_error("chapter %u vcn %" PRIu64 " is out of phase (%u)",
			      chapter_number,
			      last_vcn,
