				   ...
				   struct delta_index_page *chapter_index_page)
{
	...
	uds_log_warning("Index page map updated to %" PRIu64,
			get_last_update(volume->index_page_map));
	uds_log_warning("Page map expects that chapter %u page %u has range %u to %u, but chapter index page has chapter %" PRIu64 " with range %u to %u",
			chapter,
			index_page_number,
			bounds.lowest_list,
			bounds.highest_list,
		   ...
		   enum invalidation_reason reason)
{
	...
	unsigned int physical_chapter =
		map_to_physical_chapter(volume->geometry, virtual_chapter);
	uds_log_debug("forgetting chapter %" PRIu64, virtual_chapter);
	lock_mutex(&volume->read_threads_mutex);
