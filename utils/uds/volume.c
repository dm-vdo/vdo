 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/uds-releases/krusty/src/uds/volume.c#28 $
 */
					 ...
					 Request *request)
{
	...
	while (read_queue_is_full(volume->page_cache)) {
		...
		signalCond(&volume->read_threads_cond);
		waitCond(&volume->read_threads_read_done_cond,
				 ...
				 bool *invalid)
{
	while (((volume->reader_state & READER_STATE_EXIT) == 0) &&
		...
		!reserve_read_queue_entry(volume->page_cache,
					  ...
					  physical_page,
					  invalid))) {
		waitCond(&volume->read_threads_cond,
			 &volume->read_threads_mutex);
	}
}
				   ...
				   struct delta_index_page *chapter_index_page)
{
	...
	uint64_t ci_virtual = chapter_index_page->virtual_chapter_number;
	unsigned int ci_chapter =
		map_to_physical_chapter(geometry, ci_virtual);
	...
	logWarning("Page map expects that chapter %u page %u has range %u to %u, but chapter index page has chapter %" PRIu64 " with range %u to %u",
...
static void read_thread_function(void *arg)
{
	...
	lockMutex(&volume->read_threads_mutex);
	while (true) {
		...
		bool record_page =
			is_record_page(volume->geometry, physical_page);
		...
		if (!invalid) {
			...
			result = select_victim_in_cache(volume->page_cache,
							&page);
			if (result == UDS_SUCCESS) {
				unlockMutex(&volume->read_threads_mutex);
				result =
					read_volume_page(&volume->volume_store,
							 physical_page,
							 &page->cp_page_data);
				if (result != UDS_SUCCESS) {
					logWarning("Error reading page %u from volume",
						   physical_page);
				...
				if (!volume->page_cache->read_queue[queue_pos]
					...
					if (result == UDS_SUCCESS) {
						result = put_page_in_cache(volume->page_cache,
									   ...
									   page);
						if (result != UDS_SUCCESS) {
							logWarning("Error putting page %u in cache",
								   physical_page);
							cancel_page_in_cache(volume->page_cache,
									     physical_page,
									     page);
					...
					logWarning("Page %u invalidated after read",
						   physical_page);
					cancel_page_in_cache(volume->page_cache,
							     physical_page,
							     page);
		...
		volume->busy_reader_threads--;
		broadcastCond(&volume->read_threads_read_done_cond);
	}
	unlockMutex(&volume->read_threads_mutex);
		...
		if (!is_record_page(volume->geometry, physical_page)) {
			...
			if (result != UDS_SUCCESS) {
				if (volume->lookup_mode !=
				    LOOKUP_FOR_REBUILD) {
					logWarning("Corrupt index page %u",
						   physical_page);
				}
				cancel_page_in_cache(volume->page_cache,
						     physical_page,
		    ...
		    struct delta_index_page **index_page_ptr)
{
	...
	lockMutex(&volume->read_threads_mutex);
	struct cached_page *page = NULL;
	int result = get_volume_page_locked(volume, NULL, physical_page,
					    probe_type, &page);
	unlockMutex(&volume->read_threads_mutex);
				    ...
				    int *record_page_number)
{
	...
	struct cached_page *page = NULL;
	int result = get_volume_page_protected(volume,
					       request,
					       physical_page,
			      ...
			      bool *found)
{
	*found = false;
	...
	struct geometry *geometry = volume->geometry;
	int result = ASSERT(((record_page_number >= 0) &&
				   ...
				   struct delta_index_page index_pages[])
{
	...
	for (i = 0; i < geometry->index_pages_per_chapter; i++) {
		...
		byte *index_page = get_page_data(&volume_pages[i]);
...
{
	logDebug("forgetting chapter %" PRIu64, virtual_chapter);
	unsigned int physical_chapter =
		map_to_physical_chapter(volume->geometry, virtual_chapter);
	lockMutex(&volume->read_threads_mutex);
	int result = invalidate_page_cache_for_chapter(volume->page_cache,
						       physical_chapter,
				    ...
				    struct volume_page *scratch_page)
{
	...
	if (result != UDS_SUCCESS) {
		logWarning("Error putting page %u in cache", physical_page);
		cancel_page_in_cache(volume->page_cache, physical_page, page);
		return result;
	}
		      ...
		      byte **pages)
{
	...
	for (index_page_number = 0;
		...
		bool last_page = ((index_page_number + 1) ==
				  geometry->index_pages_per_chapter);
			 ...
			 uint64_t *virtual_chapter_number)
{
	...
	for (i = 0; i < geometry->index_pages_per_chapter; ++i) {
		...
		if (last_vcn == UINT64_MAX) {
			...
			logError("inconsistent chapter %u index page %u: expected vcn %" PRIu64 ", got vcn %" PRIu64,
				 chapter_number, i, last_vcn, vcn);
			return UDS_CORRUPT_COMPONENT;
		}

		if (expected_list_number != page->lowest_list_number) {
			logError("inconsistent chapter %u index page %u: expected list number %u, got list number %u",
	...
	if (last_vcn == UINT64_MAX) {
		logError("no chapter %u virtual chapter number determined",
			 chapter_number);
		return UDS_CORRUPT_COMPONENT;
	}
					...
					void *aux)
{
	...
	uint64_t first_vcn = UINT64_MAX;

	// doesn't matter if this results in a bad spot (UINT64_MAX)
	int result = (*probe_func)(aux, 0, &first_vcn);
	if (result != UDS_SUCCESS) {
		return UDS_SUCCESS;
	}
	...
	unsigned int right_chapter = chapter_limit;
	...
	for (;;) {
		...
		if (++bad_chapters >= max_bad_chapters) {
			logError("too many bad chapters in volume: %u",
				 bad_chapters);
			return UDS_CORRUPT_COMPONENT;
		}
	}
		...
		struct volume **new_volume)
{
	...
	int result = allocate_volume(config, layout, read_queue_max_size,
				     zone_count, &volume);
	if (result != UDS_SUCCESS) {
		return result;
	}
	result = initMutex(&volume->read_threads_mutex);
	if (result != UDS_SUCCESS) {
		free_volume(volume);
		return result;
	}
	result = initCond(&volume->read_threads_read_done_cond);
	if (result != UDS_SUCCESS) {
		free_volume(volume);
		return result;
	}
	result = initCond(&volume->read_threads_cond);
	if (result != UDS_SUCCESS) {
		free_volume(volume);
		return result;
	}
	...
	for (i = 0; i < volume_read_threads; i++) {
		result = createThread(read_thread_function,
				      (void *) volume,
				      "reader",
...
void free_volume(struct volume *volume)
{
	...
	if (volume->reader_threads != NULL) {
		...
		broadcastCond(&volume->read_threads_cond);
		unlockMutex(&volume->read_threads_mutex);
		unsigned int i;
		for (i = 0; i < volume->num_read_threads; i++) {
			joinThreads(volume->reader_threads[i]);
		}
	...
	destroyCond(&volume->read_threads_cond);
	destroyCond(&volume->read_threads_read_done_cond);
	destroyMutex(&volume->read_threads_mutex);
