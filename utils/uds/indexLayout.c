 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/uds-releases/krusty/src/uds/indexLayout.c#3 $
 */
...
computeSizes(SaveLayoutSizes *sls,
	     ...
	     unsigned int numCheckpoints)
{
	...
	result = computeMasterIndexSaveBlocks(
		&sls->config, blockSize, &sls->masterIndexBlocks);
	if (result != UDS_SUCCESS) {
		return logErrorWithStringError(
			result, "cannot compute index save size");
	}

	sls->pageMapBlocks = blockCount(
		computeIndexPageMapSaveSize(&sls->geometry), blockSize);
	sls->openChapterBlocks = blockCount(
		computeSavedOpenChapterSize(&sls->geometry), blockSize);
	sls->saveBlocks = 1 + (sls->masterIndexBlocks + sls->pageMapBlocks +
			       sls->openChapterBlocks);
	sls->subIndexBlocks =
		sls->volumeBlocks + (sls->numSaves * sls->saveBlocks);
			...
			uint64_t *indexSize)
{
	...
	int result =
		computeSizes(&sizes, config, UDS_BLOCK_SIZE, numCheckpoints);
	if (result != UDS_SUCCESS) {
		return result;
	}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
openLayoutReader(IndexLayout *layout,
		 LayoutRegion *lr,
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
openLayoutWriter(IndexLayout *layout,
		 LayoutRegion *lr,
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
decodeIndexSaveData(Buffer *buffer, IndexSaveData *saveData)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
decodeRegionHeader(Buffer *buffer, RegionHeader *header)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
decodeLayoutRegion(Buffer *buffer, LayoutRegion *region)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
loadRegionTable(BufferedReader *reader, RegionTable **tablePtr)
{
	Buffer *buffer;
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
decodeSuperBlockData(Buffer *buffer, SuperBlockData *super)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
readSuperBlockData(BufferedReader *reader,
		   SuperBlockData *super,
		   size_t savedSize)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
allocateSingleFileParts(IndexLayout *layout, SuperBlockData *super)
{
	int result = ALLOCATE(super->maxSaves,
			      IndexSaveLayout,
			      __func__,
			      &layout->index.saves);
	if (result != UDS_SUCCESS) {
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
loadSuperBlock(IndexLayout *layout,
	       size_t blockSize,
	       uint64_t firstBlock,
	       BufferedReader *reader)
{
	...
	result = readSuperBlockData(
		reader, &superBlockData, table->header.payload);
	if (result != UDS_SUCCESS) {
		FREE(table);
		return logErrorWithStringError(result,
	...
	result = reconstituteSingleFileLayout(
		layout, &superBlockData, table, firstBlock);
	FREE(table);
	return result;
}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
readIndexSaveData(BufferedReader *reader,
		  IndexSaveData *saveData,
		  size_t savedSize,
		  Buffer **bufferPtr)
{
	...
	if (saveData->version != 0) {
		...
		if (savedSize > 0) {
			result = readFromBufferedReader(
				reader, getBufferContents(buffer), savedSize);
			if (result != UDS_SUCCESS) {
				freeBuffer(&buffer);
				return result;
			}
...
iterError(RegionIterator *iter, const char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
				    ...
				    IndexSaveType saveType)
{
	...
	setupLayout(
		&isl->header, &nextBlock, 1, RL_KIND_HEADER, RL_SOLE_INSTANCE);
	setupLayout(&isl->indexPageMap,
		    &nextBlock,
		    super->pageMapBlocks,
	...
	if (saveType == IS_SAVE && isl->openChapter != NULL) {
		setupLayout(isl->openChapter,
			    &nextBlock,
			    super->openChapterBlocks,
			    RL_KIND_OPEN_CHAPTER,
			    RL_SOLE_INSTANCE);
	}
	setupLayout(&isl->freeSpace,
		    &nextBlock,
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
reconstructIndexSave(IndexSaveLayout *isl,
		     IndexSaveData *saveData,
		     SuperBlockData *super,
		     RegionTable *table)
{
	...
	if (!expectLayout(false,
		...
		iter.nextBlock =
			isl->freeSpace.startBlock + isl->freeSpace.numBlocks;
	}
	...
	if (iter.nextBlock !=
	    isl->indexSave.startBlock + isl->indexSave.numBlocks) {
		return logErrorWithStringError(
			UDS_UNEXPECTED_RESULT,
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
loadIndexSave(IndexSaveLayout *isl,
	      SuperBlockData *super,
	      BufferedReader *reader,
	      unsigned int saveId)
{
	...
	if (result != UDS_SUCCESS) {
		return logErrorWithStringError(
			result, "cannot read index 0 save %u header", saveId);
	}
	...
	result = readIndexSaveData(reader,
				   &indexSaveData,
	...
	if (result != UDS_SUCCESS) {
		FREE(table);
		return logErrorWithStringError(
			result, "unknown index 0 save %u data format", saveId);
	}
	...
	if (result != UDS_SUCCESS) {
		...
		return logErrorWithStringError(
			result, "cannot reconstruct index 0 save %u", saveId);
	}
	isl->read = true;
	return UDS_SUCCESS;
}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
loadSubIndexRegions(IndexLayout *layout)
{
	unsigned int j;
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
resetIndexSaveLayout(IndexSaveLayout *isl,
		     ...
		     unsigned int instance)
{
	...
	isl->saveType = NO_SAVE;
	setupLayout(&isl->indexSave,
		    &startBlock,
		    saveBlocks,
		    RL_KIND_SAVE,
		    instance);
	setupLayout(&isl->header,
		    nextBlockPtr,
		    1,
		    RL_KIND_HEADER,
		    RL_SOLE_INSTANCE);
				...
				unsigned int indexId)
{
	...
	sil->nonce =
		generateSecondaryNonce(masterNonce, buffer, sizeof(buffer));
	if (sil->nonce == 0) {
		sil->nonce = generateSecondaryNonce(
			~masterNonce + 1, buffer, sizeof(buffer));
	}
}

/*****************************************************************************/
__attribute__((warn_unused_result)) static int
setupSubIndex(SubIndexLayout *sil,
	      uint64_t *nextBlockPtr,
	      SaveLayoutSizes *sls,
	      unsigned int instance,
	      uint64_t masterNonce)
{
	...
	setupLayout(&sil->subIndex,
		    &startBlock,
		    sls->subIndexBlocks,
		    RL_KIND_INDEX,
		    instance);
	setupLayout(&sil->volume,
		    nextBlockPtr,
		    sls->volumeBlocks,
		    RL_KIND_VOLUME,
		    RL_SOLE_INSTANCE);
	...
	for (i = 0; i < sls->numSaves; ++i) {
		int result = resetIndexSaveLayout(&sil->saves[i],
						  nextBlockPtr,
						  sls->saveBlocks,
						  sls->pageMapBlocks,
						  i);
 ...
 **/
__attribute__((warn_unused_result)) static int
initSingleFileLayout(IndexLayout *layout,
		     uint64_t offset,
		     uint64_t size,
		     SaveLayoutSizes *sls)
{
	...
	generateSuperBlockData(sls->blockSize,
			       sls->numSaves,
			       sls->openChapterBlocks,
			       sls->pageMapBlocks,
			       &layout->super);
	...
	setupLayout(&layout->header,
		    &nextBlock,
		    1,
		    RL_KIND_HEADER,
		    RL_SOLE_INSTANCE);
	setupLayout(&layout->config,
		    &nextBlock,
		    1,
		    RL_KIND_CONFIG,
		    RL_SOLE_INSTANCE);
	result = setupSubIndex(
		&layout->index, &nextBlock, sls, 0, layout->super.nonce);
	if (result != UDS_SUCCESS) {
		return result;
	}
	setupLayout(
		&layout->seal, &nextBlock, 1, RL_KIND_SEAL, RL_SOLE_INSTANCE);
 ...
 **/
__attribute__((warn_unused_result)) static int
reconstituteSingleFileLayout(IndexLayout *layout,
			     SuperBlockData *super,
			     RegionTable *table,
			     uint64_t firstBlock)
{
	...
	RegionIterator iter = { .nextRegion = table->regions,
				.lastRegion = table->regions +
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
saveSubIndexRegions(IndexLayout *layout)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
makeSingleFileRegionTable(IndexLayout *layout,
			  unsigned int *numRegionsPtr,
			  RegionTable **tablePtr)
{
	unsigned int numRegions = 1 + // header
				  1 + // config
				  1 + // index
				  1 + // volume
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
encodeIndexSaveData(Buffer *buffer, IndexSaveData *saveData)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
encodeRegionHeader(Buffer *buffer, RegionHeader *header)
{
	...
	result = ASSERT_LOG_ONLY(contentLength(buffer) - startingLength ==
					 sizeof(*header),
				 "%zu bytes encoded, of %zu expected",
				 contentLength(buffer) - startingLength,
				 sizeof(*header));
	return result;
}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
encodeLayoutRegion(Buffer *buffer, LayoutRegion *region)
{
	size_t startingLength = contentLength(buffer);
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
encodeSuperBlockData(Buffer *buffer, SuperBlockData *super)
{
	...
	result =
		ASSERT_LOG_ONLY(contentLength(buffer) == sizeof(SuperBlockData),
				"%zu bytes encoded, of %zu expected",
				contentLength(buffer),
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
writeSingleFileHeader(IndexLayout *layout,
		      RegionTable *table,
		      ...
		      BufferedWriter *writer)
{
	...
	size_t tableSize =
		sizeof(RegionTable) + numRegions * sizeof(LayoutRegion);
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
saveSingleFileConfiguration(IndexLayout *layout)
{
...
{
	off_t offset =
		layout->index.volume.startBlock * layout->super.blockSize;
	return makeBufio(
		layout->factory, offset, blockSize, reservedBuffers, clientPtr);
}
#else
/*****************************************************************************/
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
selectOldestIndexSaveLayout(SubIndexLayout *sil,
			    unsigned int maxSaves,
			    IndexSaveLayout **islPtr)
{
	...
	for (isl = sil->saves; isl < sil->saves + maxSaves; ++isl) {
		uint64_t saveTime = 0;
		int result =
			validateIndexSaveLayout(isl, sil->nonce, &saveTime);
		if (result != UDS_SUCCESS) {
			saveTime = 0;
		}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
selectLatestIndexSaveLayout(SubIndexLayout *sil,
			    unsigned int maxSaves,
			    IndexSaveLayout **islPtr)
{
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
instantiateIndexSaveLayout(IndexSaveLayout *isl,
			   SuperBlockData *super,
			   uint64_t volumeNonce,
			   unsigned int numZones,
			   IndexSaveType saveType)
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
invalidateOldSave(IndexLayout *layout, IndexSaveLayout *isl)
{
	...
	int result = resetIndexSaveLayout(isl,
					  &startBlock,
					  saveBlocks,
					  layout->super.pageMapBlocks,
					  save);
	if (result != UDS_SUCCESS) {
		return result;
	}
		       ...
		       unsigned int *saveSlotPtr)
{
	...
	int result =
		selectOldestIndexSaveLayout(sil, layout->super.maxSaves, &isl);
	if (result != UDS_SUCCESS) {
		return result;
	}
	...
	result = instantiateIndexSaveLayout(
		isl, &layout->super, sil->nonce, numZones, saveType);
	if (result != UDS_SUCCESS) {
		return result;
	}
			    ...
			    unsigned int *slotPtr)
{
	...
	IndexSaveLayout *isl = NULL;
	int result =
		selectLatestIndexSaveLayout(sil, layout->super.maxSaves, &isl);
	if (result != UDS_SUCCESS) {
		return result;
	}
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
makeIndexSaveRegionTable(IndexSaveLayout *isl,
			 unsigned int *numRegionsPtr,
			 RegionTable **tablePtr)
{
	unsigned int numRegions = 1 + // header
				  ...
				  (bool) isl->openChapter; // open chapter if
							   // needed
...
/*****************************************************************************/
__attribute__((warn_unused_result)) static int
writeIndexSaveHeader(IndexSaveLayout *isl,
		     RegionTable *table,
		     unsigned int numRegions,
		     BufferedWriter *writer)
{
	...
	if (all) {
		...
		for (i = 0; i < layout->super.maxSaves; ++i) {
			...
			result = firstError(result,
					    invalidateOldSave(layout, isl));
		}
	} else {
		IndexSaveLayout *isl;
		result = selectLatestIndexSaveLayout(
			sil, layout->super.maxSaves, &isl);
		if (result == UDS_SUCCESS) {
			result = invalidateOldSave(layout, isl);
		}
	}
...
{
	int result = ASSERT(
		(slot < layout->super.maxSaves), "%s not started", operation);
	if (result != UDS_SUCCESS) {
		return result;
	}
