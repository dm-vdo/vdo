	...
	for (i = 0; i < num_readers; i++) {
		...
		if (i == 0) {
			...
			virtual_chapter_high = header.virtual_chapter_high;
			   ...
			   header.virtual_chapter_high) {
			return logWarningWithStringError(UDS_CORRUPT_COMPONENT,
							 "Inconsistent master index zone files: Chapter range is [%" PRIu64 ",%" PRIu64 "], chapter range %d is [%" PRIu64 ",%" PRIu64 "]",
							 virtual_chapter_low,
		...
		if (used_bits > mi5->max_zone_bits) {
			...
			if (expire_count == 1) {
				log_ratelimit(log_info,
					      "masterZone %u:  At chapter %" PRIu64 ", expiring chapter %" PRIu64 " early",
					      zone_number,
					      virtual_chapter,
				...
				log_ratelimit(log_info,
					      "masterZone %u:  At chapter %" PRIu64 ", expiring chapters %" PRIu64 " to %" PRIu64 " early",
					      zone_number,
					      virtual_chapter,
					      first_expired,
				  ...
				  uint64_t virtual_chapter)
{
	...
	for (z = 0; z < mi5->num_zones; z++) {
		...
		if (log_move) {
			log_debug("masterZone %u: The range of indexed chapters is moving from [%" PRIu64 ", %" PRIu64 "] ...",
				  z,
				  master_zone->virtual_chapter_low,
				  master_zone->virtual_chapter_high);
		}
