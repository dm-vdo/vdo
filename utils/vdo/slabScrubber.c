 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/slabScrubber.c#38 $
 */
	...
	while (entry_point.entry_count < entry_count) {
		...
		if (entry.sbn > max_sbn) {
			...
			return logErrorWithStringError(VDO_CORRUPT_JOURNAL,
						       "vdo_slab journal entry (%" PRIu64 ", %u) had invalid offset %u in slab (size %u blocks)",
						       block_number,
						       entry_point.entry_count,
						       entry.sbn,
						       max_sbn);
		...
		if (result != VDO_SUCCESS) {
			logErrorWithStringError(result,
						"vdo_slab journal entry (%" PRIu64 ", %u) (%s of offset %" PRIu32 ") could not be applied in slab %u",
						block_number,
						entry_point.entry_count,
...
static void apply_journal_entries(struct vdo_completion *completion)
{
	...
	sequence_number_t head =
		get_unaligned_le64(end_block->header.fields.head);
