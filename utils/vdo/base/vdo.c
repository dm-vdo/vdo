 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/vdo.c#62 $
 */
			...
			bool require_logical)
{
	...
	if (block_count < config->physical_blocks) {
		logError("A physical size of %" PRIu64 " blocks was specified, but that is smaller than the %" PRIu64 " blocks configured in the vdo super block",
			 block_count,
			 config->physical_blocks);
					...
					LogicalBlockNumber lbn)
{
 ...
 ", state %u. Giving up on deduplication of logical block %" PRIu64,
	...
	if ((result != VDO_SUCCESS) || (zone == NULL)) {
		logDebug("Invalid physical block number from deduplication server: %" PRIu64 ", giving up on deduplication of logical block %" PRIu64,
			 advice->pbn,
			 lbn);
		atomicAdd64(&vdo->error_stats.invalid_advice_pbn_count, 1);
		return no_advice;
