 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/packer.c#36 $
 */
...
static void complete_output_bin(struct vdo_completion *completion)
{
	...
	if (completion->result != VDO_SUCCESS) {
		updateVIOErrorStats(vio,
				    "Completing compressed write vio for physical block %" PRIu64 " with error",
				    vio->physical);
	}
...
{
	if (is_read_only(get_vdo_from_allocating_vio(bin->writer)->readOnlyNotifier)) {
...
{
	logInfo("packer");
	logInfo("  flushGeneration=%" PRIu64 " state %s writing_batches=%s",
		packer->flush_generation, get_admin_state_name(&packer->state),
		boolToString(packer->writing_batches));

	logInfo("  input_bin_count=%" PRIu64, packer->size);
	struct input_bin *bin;
	for (bin = get_fullest_bin(packer); bin != NULL;
	     bin = next_bin(packer, bin)) {
