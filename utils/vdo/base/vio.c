...
void update_vio_error_stats(struct vio *vio, const char *format, ...)
{
	...
	switch (result) {
	case VDO_READ_ONLY:
		atomicAdd64(&vio->vdo->error_stats.read_only_error_count, 1);
		return;

	case VDO_NO_SPACE:
		atomicAdd64(&vio->vdo->error_stats.no_space_error_count, 1);
		priority = LOG_DEBUG;
		break;
...
static void handle_metadata_io_error(struct vdo_completion *completion)
{
	...
	update_vio_error_stats(vio,
			       "Completing %s vio of type %u for physical block %" PRIu64 " with error",
