			   ...
			   struct slab_depot_state_2_0 *state)
{
	...
	logDebug("slabDepot configure_state(block_count=%" PRIu64 ", first_block=%" PRIu64 ", slab_size=%" PRIu64 ", zone_count=%u)",
		 block_count,
		 first_block,
		 slab_size,
		 zone_count);
	...
	*state = (struct slab_depot_state_2_0) {
	...
	logDebug("slab_depot last_block=%" PRIu64 ", total_data_blocks=%" PRIu64 ", slab_count=%zu, left_over=%" PRIu64,
		 last_block,
		 total_data_blocks,
		 slab_count,
...
void dump_slab_depot(const struct slab_depot *depot)
{
	...
	logInfo("  zone_count=%u old_zone_count=%u slabCount=%" PRIu32 " active_release_request=%" PRIu64 " new_release_request=%" PRIu64,
