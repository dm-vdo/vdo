 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/vdo.c#80 $
 */
					...
					logical_block_number_t lbn)
{
	...
	if ((advice->state == MAPPING_STATE_UNMAPPED) ||
		...
		log_debug("Invalid advice from deduplication server: pbn %" PRIu64 ", state %u. Giving up on deduplication of logical block %" PRIu64,
			  advice->pbn,
			  advice->state,
			  lbn);
	...
	if ((result != VDO_SUCCESS) || (zone == NULL)) {
		log_debug("Invalid physical block number from deduplication server: %" PRIu64 ", giving up on deduplication of logical block %" PRIu64,
			  advice->pbn,
			  lbn);
		atomicAdd64(&vdo->error_stats.invalid_advice_pbn_count, 1);
		return no_advice;
