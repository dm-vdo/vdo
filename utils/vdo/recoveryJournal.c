...
static void continue_committed_waiter(struct waiter *waiter, void *context)
{
	...
	ASSERT_LOG_ONLY(before_journal_point(&journal->commit_point,
			...
			"DataVIOs released from recovery journal in order. Recovery journal point is (%" PRIu64 ", %u), but commit waiter point is (%" PRIu64 ", %u)",
...
void dump_recovery_journal_statistics(const struct recovery_journal *journal)
{
	...
	log_info("  block_map_head=%" PRIu64 " slab_journal_head=%" PRIu64 " last_write_acknowledged=%" PRIu64 " tail=%" PRIu64 " block_map_reap_head=%" PRIu64 " slab_journal_reap_head=%" PRIu64 " disk_full=%" PRIu64 " slab_journal_commits_requested=%" PRIu64 " increment_waiters=%zu decrement_waiters=%zu",
	...
	log_info("  entries: started=%" PRIu64 " written=%" PRIu64 " committed=%" PRIu64,
		 stats.entries.started, stats.entries.written,
		 stats.entries.committed);
	log_info("  blocks: started=%" PRIu64 " written=%" PRIu64 " committed=%" PRIu64,
