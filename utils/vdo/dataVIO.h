 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/dataVIO.h#50 $
 */
...
static inline void assert_in_hash_zone(struct data_vio *data_vio)
{
	...
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for logical block %" PRIu64 " on thread %u, should be on hash zone thread %u",
...
static inline void assert_in_logical_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for logical block %" PRIu64 " on thread %u, should be on thread %u",
			data_vio->logical.lbn,
			thread_id,
			expected);
}
...
static inline void assert_in_duplicate_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for duplicate physical block %" PRIu64 " on thread %u, should be on thread %u",
...
static inline void assert_in_mapped_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for mapped physical block %" PRIu64 " on thread %u, should be on thread %u",
			data_vio->mapped.pbn,
			thread_id,
			expected);
}
...
static inline void assert_in_new_mapped_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for new_mapped physical block %" PRIu64 " on thread %u, should be on thread %u",
...
static inline void assert_in_journal_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for logical block %" PRIu64 " on thread %u, should be on journal thread %u",
			data_vio->logical.lbn,
			thread_id,
			expected);
}
...
static inline void assert_in_packer_zone(struct data_vio *data_vio)
{
	...
	thread_id_t thread_id = getCallbackThreadID();
	ASSERT_LOG_ONLY((expected == thread_id),
			"data_vio for logical block %" PRIu64 " on thread %u, should be on packer thread %u",
			data_vio->logical.lbn,
			thread_id,
			expected);
}
