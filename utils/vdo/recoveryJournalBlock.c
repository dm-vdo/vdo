 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/recoveryJournalBlock.c#34 $
 */
		       ...
		       physical_block_number_t *pbn_ptr)
{
	...
	if (result != VDO_SUCCESS) {
		logErrorWithStringError(result,
					"Error translating recovery journal block number %" PRIu64,
					block->block_number);
	}
	return result;
}
...
{
	logInfo("    sequence number %" PRIu64 "; entries %" PRIu16 "; %s; %zu entry waiters; %zu commit waiters",
