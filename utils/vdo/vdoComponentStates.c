 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/vdoComponentStates.c#7 $
 */
			    ...
			    struct vdo_component_states *states)
{
	...
	result = decode_version_number(buffer, &states->master_version);
	if (result != VDO_SUCCESS) {
		return result;
	}

	result = validate_version(VDO_MASTER_VERSION_67_0,
				  states->master_version,
				  "master");
	if (result != VDO_SUCCESS) {
		return result;
	}
			      ...
			      block_count_t size)
{
	if (geometry_nonce != states->vdo.nonce) {
		return log_error_strerror(VDO_BAD_NONCE,
					  "Geometry nonce %" PRIu64 " does not match superblock nonce %" PRIu64,
					  geometry_nonce,
					  states->vdo.nonce);
	}
