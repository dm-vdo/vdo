 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/slabJournal.c#62 $
 */
	...
	if (result != VDO_SUCCESS) {
		if (result != VDO_READ_ONLY) {
			...
			logErrorWithStringError(result,
						"failed slab summary update %" PRIu64,
						journal->summarized);
		}
...
static void complete_write(struct vdo_completion *completion)
{
	...
	if (write_result != VDO_SUCCESS) {
		logErrorWithStringError(write_result,
		      ...
		      const struct journal_point *recovery_point)
{
	int result =
		ASSERT(before_journal_point(&journal->tail_header.recovery_point,
					    recovery_point),
		       "recovery journal point is monotonically increasing, recovery point: %" PRIu64 ".%u, block recovery point: %" PRIu64 ".%u",
					 ...
					 int adjustment)
{
	...
	if (adjustment < 0) {
		ASSERT_LOG_ONLY((-adjustment <= lock->count),
				"adjustment %d of lock count %u for slab journal block %" PRIu64 " must not underflow",
...
{
	logInfo("  slab journal: entry_waiters=%zu waiting_to_commit=%s" " updating_slab_summary=%s head=%" PRIu64 " unreapable=%" PRIu64 " tail=%" PRIu64 " next_commit=%" PRIu64 " summarized=%" PRIu64 " last_summarized=%" PRIu64 " recovery_lock=%" PRIu64 " dirty=%s",
		...
		boolToString(is_slab_journal_dirty(journal)));
