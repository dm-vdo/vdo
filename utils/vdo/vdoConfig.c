 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/user/vdoConfig.c#36 $
 */
...
static int __must_check configureVDO(UserVDO *vdo)
{
  ...
  result = configure_slab_depot(get_fixed_layout_partition_size(partition),
                                ...
                                UUID                       uuid)
{
  ...
  if (result != VDO_SUCCESS) {
    return logErrorWithStringError(result, "cannot clear block map partition");
  }
