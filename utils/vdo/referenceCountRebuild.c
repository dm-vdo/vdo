 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/referenceCountRebuild.c#38 $
 */
	...
	for (slot = 0; slot < BLOCK_MAP_ENTRIES_PER_PAGE; slot++) {
		...
		if (result != VDO_SUCCESS) {
			logErrorWithStringError(result,
						"Could not adjust reference count for PBN %" PRIu64 ", slot %u mapped to PBN %" PRIu64,
			...
			struct vdo_completion *completion)
{
	...
	if (result != VDO_SUCCESS) {
		return logErrorWithStringError(result,
					       "Could not adjust reference count for block map tree PBN %" PRIu64,
					       pbn);
	}
