 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/slabDepotFormat.c#10 $
 */
			 ...
			 struct slab_depot_state_2_0 *state)
{
	...
	log_debug("slabDepot configure_slab_depot(block_count=%" PRIu64 ", first_block=%" PRIu64 ", slab_size=%" PRIu64 ", zone_count=%u)",
		  block_count,
		  first_block,
		  slab_size,
		  zone_count);
	...
	*state = (struct slab_depot_state_2_0) {
	...
	log_debug("slab_depot last_block=%" PRIu64 ", total_data_blocks=%" PRIu64 ", slab_count=%zu, left_over=%" PRIu64,
		  last_block,
		  total_data_blocks,
		  slab_count,
